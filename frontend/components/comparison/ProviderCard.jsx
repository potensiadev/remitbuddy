...
const ProviderCard = ({ provider, isBest, index, onProviderClick, bestAmount, worstAmount, sendAmount }) => {
  ...
  return (
    <div
      className={`relative w-full rounded-2xl transition-all duration-300 md:mb-6 mb-4 ${isBest
        ? 'bg-white border-2 border-blue-500 shadow-xl z-10 mt-8 md:mt-0' // Added top margin for mobile badge clearance
    ...
    >
      ...
      {isBest && (
        <div className="absolute -top-8 left-0 right-0 bg-blue-500 text-white py-1.5 rounded-t-xl font-bold text-sm text-center shadow-sm flex md:hidden justify-center">
          Best Rate Provider
        </div>
      )}
      ...
      <div className="p-5 flex flex-col md:flex-row items-center gap-6">
        ...
        {/* 1. Identity Column */}
        <div className="flex flex-col items-center justify-center md:w-[15%] gap-2">
          <h3 className={`font-black text-xl md:text-2xl ${isBest ? 'text-blue-600' : 'text-slate-700'}`}>
            {displayName}
          </h3>
          {/* Mobile-only logo if needed, but text name is cleaner as per mockup */}
        </div>

        {/* 2. Data Grid - The Core Info */}
        <div className="w-full md:w-[60%] grid grid-cols-1 gap-y-3 pl-0 md:pl-8 md:border-l md:border-gray-100 border-t border-gray-100 md:border-t-0 pt-4 md:pt-0">

          {/* Row 1: Recipient Gets (Hero) */}
          <div className="flex justify-between items-center">
            <span className={`text-sm font-bold ${isBest ? 'text-blue-600' : 'text-gray-600'}`}>
              Recipient Get
            </span>
        ...
        <div className="w-full md:w-[25%] flex justify-end">
          <a
            ...
            className={`flex items-center justify-center gap-2 w-full py-3 md:py-4 rounded-lg font-bold text-base transition-all transform active:scale-95
              ${isBest
