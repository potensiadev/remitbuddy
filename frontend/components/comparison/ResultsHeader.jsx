...
export default function ResultsHeader({
  ...
  const formatLastUpdated = (date) => {
    if (!date) return t('results.just_now', 'Just now');
    const now = new Date();
    const updated = new Date(date);
    const diffMs = now - updated;
    const diffMins = Math.floor(diffMs / 60000);

    if (diffMins < 1) return t('results.just_now', 'Just now');
    if (diffMins < 60) return t('results.minutes_ago', '{{minutes}}m ago').replace('{{minutes}}', diffMins);
    const diffHours = Math.floor(diffMins / 60);
    if (diffHours < 24) return t('results.hours_ago', '{{hours}}h ago').replace('{{hours}}', diffHours);
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                <h1 className="
                  ...
                  tracking-tight
                ">
                  {country?.name} {t('compare.title_suffix', 'Remittance')}
                </h1>
                <p className="text-sm text-neutral-500 font-medium">
                  {t('compare.subtitle', 'Real-time Rate Comparison')}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 text-sm text-neutral-500">
            ...
            {providerCount > 0 && (
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  {t('results.providers_compared', '{{count}} providers compared').replace('{{count}}', providerCount)}
                </span>
              </div>
            )}
          </div>
        ...
        ">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-accent-700 font-semibold">
              {t('results.savings_message', 'Save up to ₩{{savings}}!').replace('{{savings}}', savings.toLocaleString())}
            </span>
          </div>
        </div>
      ) : (
        ...
        ">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-primary-700 font-medium">
              {t('results.tip', 'Compare rates and fees for maximum savings')}
            </span>
          </div>
        </div>
      )}
